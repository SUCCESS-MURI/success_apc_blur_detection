...
def UNet(t_image, is_train=False, reuse=False, scope = "UNet"):
    w_init1 = tf.compat.v1.random_normal_initializer(stddev=0.02)
    w_init2 = tf.compat.v1.random_normal_initializer(stddev=0.01)
    w_init3 = tf.compat.v1.random_normal_initializer(stddev=0.005)
    w_init4 = tf.compat.v1.random_normal_initializer(stddev=0.002)
    b_init = None # tf.constant_initializer(value=0.0)
    g_init = tf.compat.v1.random_normal_initializer(1., 0.02)
...
def VGG19_pretrained(t_image,reuse = False,scope="VGG"):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse) as vs:
        ...
        f0 = Model(inputs=net_in,outputs=network,name='f0')
        #n.outputs= tf.nn.relu(n.outputs)
        ...
        f0_1 = Model(inputs=net_in,outputs=network,name='f0_1')
        #n.outputs = tf.nn.relu(n.outputs)
        ...
        f1_2 = Model(inputs=net_in,outputs=network,name='f1_2')
        #n.outputs = tf.nn.relu(n.outputs)
        ...
        f2_3 = Model(inputs=net_in,outputs=network,name='f2_3')
        #n.outputs=tf.nn.relu(n.outputs)
...
def Decoder_Network_classification(ninput,f0,f1_2,f2_3,f3_4,hrg,wrg, reuse=False, scope = "UNet"):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse) as vs:
        ...
        f3_4 = Conv2d(n_filter=512, filter_size=(1, 1), strides=(1, 1), act=None, padding='SAME', W_init=w_init4,
                      name='f3_4/c1')(f3_4)
        n = Elementwise(combine_fn=tf.add, name='s5')([n, f3_4])
        ...
        n_m3 = Conv2d(n_filter=3, filter_size=(1, 1), strides=(1, 1), act=None, padding='SAME', W_init=w_init4,
                      name='u4/loss3')(n)
        n_m3 = Model(inputs=ninput,outputs=n_m3,name='n_m3')
        #(hrg // 4, wrg // 4)
        ...
        f2_3 = Conv2d(n_filter=256, filter_size=(1, 1), strides=(1, 1), act=None, padding='SAME', W_init=w_init3,
                      name='f2_3/c1')(f2_3)
        n = Elementwise(combine_fn=tf.add, name='s4')([n, f2_3])
        # n = InputLayer(tf.nn.relu(n.outputs), name='relu4')
        n.outputs = tf.nn.relu(n)
        ...
        n_m2 = Conv2d(n_filter=3, filter_size=(1, 1), strides=(1, 1), act=None, padding='SAME', W_init=w_init3,
                      name='u3/loss2')(n)
        n_m2 = Model(inputs=ninput, outputs=n_m2, name='n_m2')
        ...
        f1_2 = Conv2d(n_filter=128, filter_size=(1, 1), strides=(1, 1), act=None, padding='SAME', W_init=w_init2,
                      name='f1_2/c1')(f1_2)
        n = Elementwise(combine_fn=tf.add, name='s3')([n, f1_2])
        # n = InputLayer(tf.nn.relu(n.outputs), name='relu3')
        n.outputs = tf.nn.relu(n)
        ...
        n_m1 = Conv2d(n_filter=3, filter_size=(1, 1), strides=(1, 1), act=None, padding='SAME', W_init=w_init2,
                      name='u2/loss1')(n)
        n_m1 = Model(inputs=ninput, outputs=n_m1, name='n_m1')
        #(hrg, wrg),
        ...
        f0 = Conv2d(n_filter=64, filter_size=(1, 1), strides=(1, 1), act=None, padding='SAME', W_init=w_init1,
                    name='f0/c1')(f0)
        n = Elementwise(combine_fn=tf.add, name='s2')([n, f0])
        n.outputs = tf.nn.relu(n)
        # n = InputLayer(tf.nn.relu(n.outputs), name='relu2')
        ...
        # n = BatchNormLayer(n, act=tf.nn.relu, is_train=is_train, gamma_init=g_init, name='u1/b1')
        # n.outputs = tf.nn.relu(n.outputs)
        n = Conv2d(n_filter=3, filter_size=(1, 1), strides=(1, 1), act=None, padding='SAME', W_init=w_init1,
                   name='u1/c5')(n)
        # n = BatchNormLayer(n, is_train=is_train, gamma_init=g_init, name='u1/b2')
        # n.outputs = tf.nn.relu(n.outputs)
        # n = ElementwiseLayer([n, n_init], tf.add, name='s1')
