...
from utils import read_all_imgs, get_imgs_RGB_fn, get_imgs_RGBGRAY_2_fn, crop_img_and_classification_fn_val
...
#from U2Net.data_loader import RescaleT, ToTensorLab
...
def validation():
    ...
    ### DEFINE MODEL ###
    patches_blurred = tf.compat.v1.placeholder('float32', [10, h, w, 3], name='input_patches')
    labels = tf.compat.v1.placeholder('int64', [10, h, w, 1], name='labels')
    with tf.compat.v1.variable_scope('Unified'):
        with tf.compat.v1.variable_scope('VGG') as scope1:
            input, n, f0, f0_1, f1_2, f2_3 = VGG19_pretrained(patches_blurred, reuse=False, scope=scope1)
        with tf.compat.v1.variable_scope('UNet') as scope2:
    ...
    # read check point weights
    # tl.files.load_ckpt(sess=sess, mode_name='SA_net_{}_best_model.ckpt'.format(tl.global_flag['mode']),
    #                    save_dir='./model/ips_dataset_3_run_1', var_list=a_vars, is_latest=True, printable=True)
    reader = py_checkpoint_reader.NewCheckpointReader('./model/SA_net_{}.ckpt'.format(tl.global_flag['mode']))
    state_dict = {v: reader.get_tensor(v) for v in reader.get_variable_to_shape_map()}
    # save weights to the model
    get_weights_checkpoint(sess, net_regression, state_dict)
    ...
    #     image_name = test_blur_img_list[i]
    #
    #     test_image, gt_test_image = crop_img_and_classification_fn_val([test_image,gt_test_image])
    #     gt_test_image = gt_test_image[:,:,0][:,:,np.newaxis]
    #     # red = test_image[:, :, 0]
    #     # green = test_image[:, :, 1]
    #     # blue = test_image[:, :, 2]
    #     # test_image = np.zeros(test_image.shape)
    ...
    for idx in range(0, len(test_blur_imgs), 10):
        images_and_score = tl.prepro.threading_data(
            [_ for _ in zip(test_blur_imgs[idx: idx + 10],
                            test_mask_imgs[
                            idx: idx + 10])],
            fn=crop_img_and_classification_fn_val)

        imlist, clist = images_and_score.transpose((1, 0, 2, 3, 4))
        clist = clist[:, :, :, 0]
        clist = np.expand_dims(clist, axis=3)

        outmap = sess.run([output_map], {net_regression.inputs: imlist})[0]
        # per class accuracy
    ...
    print(mean_accuracy_per_class)
    best_accuracy = np.mean(mean_accuracy_per_class)
    print(best_accuracy)
        # Model
        # start_time = time.time()
        # # uncertain labeling
        ...
        # all_gt_binary_results.append(test_gt_image_blur_no_blur)
        #
        # perclass_accuracy.append(perclass_accuracy_conf_matrix.diagonal())
        # # for lab in range(5):
        # #     if (perclass_accuracy_conf_matrix[lab, :] == 0).all() and (
        # #             perclass_accuracy_conf_matrix[:, lab] == 0).all():
        # #         pass
...
# our test
def test_w_saliance():
    print("Blurmap Testing with Saliency")
    ...
    # trans = transforms.Compose([transform.resize(320),
    #                             ToTensorLab(flag=0)])
    net = define_and_get_unet()
    masked_images = []

    # to allocate memory correctly
    for i in range(len(test_blur_imgs)):
        test_image = test_blur_imgs[i]
        # saliance
        # inputs_test = trans(test_image).type(torch.FloatTensor)
        image = transform.resize(test_image,(320,320),mode='constant')
        ...
        masked_image = np.array(Image.fromarray(masked_image * 255 > 1e-3).convert('RGB').resize(
            (test_image.shape[1], test_image.shape[0]), resample=Image.BILINEAR))[:,:,0]
    ...
    for i in range(len(test_blur_imgs)):
        ...
        blur_map_labels = copy.deepcopy(np.squeeze(blur_map))
        gt_test_labels = copy.deepcopy(np.squeeze(gt_test_image))
        blur_map_labels[masked_image < 1e-3] = 10
        gt_test_labels[masked_image < 1e-3] = 10
        blur_map_labels = blur_map_labels.flatten()
        gt_test_labels = gt_test_labels.flatten()
        blur_map_labels = blur_map_labels[blur_map_labels != 10]
        gt_test_labels = gt_test_labels[gt_test_labels != 10]

        accuracy = accuracy_score(blur_map_labels, gt_test_labels, normalize=True)
        # compare binary map
        blur_map_binary = np.copy(blur_map)
        blur_map_binary[blur_map > 0] = 1

        blur_map_binary_labels = copy.deepcopy(np.squeeze(blur_map_binary))
        gt_test_binary_labels = copy.deepcopy(np.squeeze(test_gt_image_blur_no_blur))
        blur_map_binary_labels[masked_image < 1e-3] = 10
        gt_test_binary_labels[masked_image < 1e-3] = 10
        blur_map_binary_labels = blur_map_binary_labels.flatten()
        gt_test_binary_labels = gt_test_binary_labels.flatten()
        blur_map_binary_labels = blur_map_binary_labels[blur_map_binary_labels != 10]
        gt_test_binary_labels = gt_test_binary_labels[gt_test_binary_labels != 10]

        accuracy_binary = accuracy_score(blur_map_binary_labels, gt_test_binary_labels, normalize=True)
        # accuracy_binary = sess.run([loss1], {output_map: blur_map_binary, labels: np.expand_dims(
        #     test_gt_image_blur_no_blur, axis=0)})[0]
        ...
        # calculate mean intersection of union
        miou = numpy_iou(gt_test_labels, blur_map_labels, 5)
        miou_binary = numpy_iou(gt_test_binary_labels, blur_map_binary_labels, 2)

        # https://stackoverflow.com/questions/39770376/scikit-learn-get-accuracy-scores-for-each-class
        perclass_accuracy_conf_matrix = confusion_matrix(gt_test_labels, blur_map_labels, labels=[0, 1, 2, 3, 4],
                                                         normalize="true")
        all_img_results.append(blur_map_labels)
        all_gt_img_results.append(gt_test_labels)
        all_img_binary_results.append(blur_map_binary_labels)
        all_gt_binary_results.append(gt_test_binary_labels)
        ...
        perclass_accuracy_conf_matrix = confusion_matrix(gt_test_binary_labels,blur_map_binary_labels,labels=[0, 1],
                                                         normalize="true")
        ...
        # calculate f1 score
        # https://machinelearningmastery.com/precision-recall-and-f-measure-for-imbalanced-classification/
        f1score = f1_score(blur_map_labels, gt_test_labels, labels=[0, 1, 2, 3, 4], average='micro')
        f1score_binary = f1_score(gt_test_binary_labels,blur_map_binary_labels, labels=[0, 1],
                                  average='micro')
    ...
    plt.clf()
    plt.figure(figsize=(12, 10))
    final_confmatrix = confusion_matrix(list(chain(*all_gt_img_results)), list(chain(*all_img_results)),
                                        labels=[0, 1, 2, 3, 4], normalize="true")
    np.save(save_dir_sample +'/all_labels_results_conf_matrix.npy', final_confmatrix)
    final_confmatrix = np.round(final_confmatrix, 2)
    cax = plt.imshow(final_confmatrix, interpolation='nearest', cmap=plt.cm.Blues)
    ...
    plt.clf()
    plt.figure(figsize=(8, 7))
    final_confmatrix = confusion_matrix(list(chain(*all_gt_binary_results)),list(chain(*all_img_binary_results)),
                                        labels=[0, 1], normalize="true")
    np.save(save_dir_sample +'/all_binary_results_conf_matrix.npy', final_confmatrix)
    final_confmatrix = np.round(final_confmatrix, 2)
    cax = plt.imshow(final_confmatrix, interpolation='nearest', cmap=plt.cm.Blues)
