...
def blurmap_3classes_using_numpy_pretrainied_weights():
    ...
    #Put the input path!
    sharp_path = '/home/mary/code/local_success_dataset/CUHK_Split_Dataset/Testing/image/'#./input'
    test_sharp_img_list = os.listdir(sharp_path)
    test_sharp_img_list.sort()

    sharp_gt_path = '/home/mary/code/local_success_dataset/CUHK_Split_Dataset/Testing/gt/'  # ./input'
    test_sharp_gt_list = os.listdir(sharp_gt_path)
    test_sharp_gt_list.sort()
    final_shape = (480, 640)

    # # Model
    # patches_blurred = tf.compat.v1.placeholder('float32', [1, final_shape[0], final_shape[1], 3],
    #                                            name='input_patches')
    ...
    plt.imshow(final_confmatrix, interpolation='nearest', cmap='GnBu')
...
def run_session(image,gt,gt_binary,return_dict):
    ...
    blur_map0,outmap1,outmap2,outmap3 = sess.run([output_map,output_map_1,output_map_2,output_map_3], {net_regression.inputs: np.expand_dims((image), axis=0)})
    ...
    accuracy1 = sess.run([loss1],{output_map:blur_map_binary,labels:np.expand_dims((gt_binary[:, :, np.newaxis]),axis=0)})[0]
...
def testData_return_error():
    ...
    test_mask_imgs = read_all_imgs(test_mask_img_list, path=config.TEST.ssc_gt_path, n_threads=100, mode='RGB2GRAY2')
