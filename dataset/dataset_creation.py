...
def create_muri_dataset_for_sensitivity(args):
    ...
    halfshape = (width, height)
    print(halfshape)
    width = int(640 * 30 / 100)
    height = int(480 * 30 / 100)
    thirdshape = (width, height)
    print(thirdshape)
    width = int(640 * 70 / 100)
    height = int(480 * 70 / 100)
    threequartersshape = (width, height)
    print(threequartersshape)

    # now for sensitivity figure out focus ranges
    focus_ranges = np.arange(3,50,6)
    # motion ranges
    motion_kernal_ranges = np.arange(3,50,6)
    motion_degree_ranges = np.arange(-180,190,30)
    # brightness ranges
    brightness_alpha = np.arange(1.5, 2.6, .3)
    brightness_beta = np.arange(0, 100, 30)
    # darkness ranges
    darkness_alpha = np.arange(0.1, 0.6, .2)
    darkness_beta = np.arange(-100, 0, 30)
    ...
    for j in range(1):
        if j == 0:
            size_name = 'seventy_percent_size_'
            current_size = threequartersshape
        if j == 1:
            size_name = 'half_size_'
            current_size = halfshape
        if j == 2:
            size_name = 'third_size_'
            current_size = thirdshape
        # baseline images
        for i in range(len(imagesOrigonal)):
            # always remember gamma correction
            baseImage = np.round(np.power((cv2.resize(imagesOrigonal[i], current_size) * 1.) / 255,
                                          (1 / gamma)) * 255.0).astype(np.uint8)
            baseImageName = images_list[i]
            saveName = args.output_data_dir + "/images/" + size_name + baseImageName + '_noblur' + args.data_extension
            imageio.imsave(saveName, baseImage)
            # get ground truth mask
            saveName = args.output_data_dir + "/gt/" + size_name + baseImageName + '_noblur' + args.data_extension
            # double check all ints are valid
            tempGT = np.round(cv2.resize(gtOrigonal[i],current_size))
            nMask = np.copy(tempGT)
            nMask[tempGT == 1] = 64
            nMask[tempGT == 2] = 128
            nMask[tempGT == 3] = 192
            nMask[tempGT == 4] = 255
        ...
        for i in range(len(imagesOrigonal)):
            baseImageName = images_list[i]
            tempImage = cv2.resize(imagesOrigonal[i], current_size)
            tempGT = np.round(cv2.resize(gtOrigonal[i], current_size))
            tempSaliency = cv2.resize(saliencyImages[i], current_size)
            tempsaliencyMask = np.round(cv2.resize(saliencyMask[i], current_size))
            for count in range(10):
                nMask = copy.deepcopy(tempGT)
                # motion blur
                kernal, angle = random_motion_blur_kernel()
                motion_blurred_overlay_img = apply_motion_blur(copy.deepcopy(tempSaliency), kernal, angle)
                blur_mask = apply_motion_blur(copy.deepcopy(tempsaliencyMask) * 255, kernal, angle)
                motion_blur_mask = copy.deepcopy(blur_mask)
                motion_blur_mask[np.round(blur_mask, 4) > 0] = 255
                alpha = blur_mask / 255.
                final_masked_blurred_image, y1o, y2o, x1o, x2o = overlay_image_alpha(copy.deepcopy(tempImage),
                                                                                     motion_blurred_overlay_img,
                                                                                     0, 0, alpha)
                # cv2.imshow('MotionImage', final_masked_blurred_image)
                # cv2.waitKey(0)
                nMask[np.argwhere(motion_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 0],
                      np.argwhere(motion_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 1]] = 1
                saveName = args.output_data_dir + "/images/"+size_name+"motion_" + baseImageName + '_' + str(count) + \
                           args.data_extension
                final_masked_blurred_image = np.round(np.power((np.array(final_masked_blurred_image)[:, :, 0:3] * 1.)
                                                               / 255.0, (1.0 / gamma)) * 255.0).astype(np.uint8)
                # https: // note.nkmk.me / en / python - opencv - bgr - rgb - cvtcolor /
                imageio.imwrite(saveName, final_masked_blurred_image)
                # create and save ground truth mask
                saveNameGt = args.output_data_dir + "/gt/"+size_name+"motion_"+ baseImageName + '_' + str(count) + \
                             args.data_extension
                nMask[nMask == 1] = 64
                nMask[nMask == 2] = 128
                nMask[nMask == 3] = 192
                ...
                nMask = copy.deepcopy(tempGT)
                kernal = random_focus_blur_kernel()
                focus_blurred_overlay_img = apply_out_of_focus_blur(copy.deepcopy(tempSaliency), kernal)
                blur_mask = apply_out_of_focus_blur(copy.deepcopy(tempsaliencyMask) * 255, kernal)
                focus_blur_mask = copy.deepcopy(blur_mask)
                focus_blur_mask[np.round(blur_mask, 4) > 0] = 255
                alpha = blur_mask / 255.
                final_masked_blurred_image, y1o, y2o, x1o, x2o = overlay_image_alpha(copy.deepcopy(tempImage),
                ...
                nMask[np.argwhere(focus_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 0],
                      np.argwhere(focus_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 1]] = 2
                saveName = args.output_data_dir + "/images/"+size_name+"focus_" + baseImageName + '_' + str(count) + \
                           args.data_extension
                final_masked_blurred_image = np.round(np.power((np.array(final_masked_blurred_image)[:, :, 0:3] * 1.)
                                                               / 255.0, (1.0 / gamma)) * 255.0).astype(np.uint8)
                # https: // note.nkmk.me / en / python - opencv - bgr - rgb - cvtcolor /
                imageio.imwrite(saveName, final_masked_blurred_image)
                # create and save ground truth mask
                saveNameGt = args.output_data_dir + "/gt/"+size_name+"focus_" + baseImageName + '_' + str(count) + \
                             args.data_extension
                nMask[nMask == 1] = 64
                nMask[nMask == 2] = 128
                nMask[nMask == 3] = 192
                ...
                nMask = copy.deepcopy(tempGT)
                alpha, beta = random_darkness_value()
                dark_blurred_overlay_img = create_brightness_and_darkness_blur(copy.deepcopy(tempSaliency), alpha,beta)
                # cv2.imshow('BrightnessImage', dark_blur)
                # cv2.waitKey(0)
                dark_blur_mask = copy.deepcopy(tempsaliencyMask)
                dark_blur_mask[dark_blur_mask > 0] = 255
                alpha_mask = (copy.deepcopy(tempsaliencyMask) * 255.0) / 255.
                final_masked_blurred_image, y1o, y2o, x1o, x2o = overlay_image_alpha(copy.deepcopy(tempImage),
                                                                                     dark_blurred_overlay_img,
                                                                                     0, 0, alpha_mask)
                # cv2.imshow('DarknessImage', final_masked_blurred_image)
                ...
                nMask[np.argwhere(dark_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 0],
                      np.argwhere(dark_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 1]] = 3
                saveName = args.output_data_dir + "/images/"+size_name+"darkness_" + baseImageName + '_' + str(count)+\
                           args.data_extension
                final_masked_blurred_image = np.round(np.power((np.array(final_masked_blurred_image)[:, :, 0:3] * 1.)
                                                               / 255.0, (1.0 / gamma)) * 255.0).astype(np.uint8)
                # https: // note.nkmk.me / en / python - opencv - bgr - rgb - cvtcolor /
                imageio.imwrite(saveName, final_masked_blurred_image)
                # create and save ground truth mask
                saveNameGt = args.output_data_dir + "/gt/"+size_name+"darkness_" + baseImageName + '_' + str(count) + \
                             args.data_extension
                nMask[nMask == 1] = 64
                nMask[nMask == 2] = 128
                nMask[nMask == 3] = 192
                ...
                # brightness blur
                alpha, beta = random_brightness_value()
                bright_blurred_overlay_img = create_brightness_and_darkness_blur(copy.deepcopy(tempSaliency), alpha,
                                                                                 beta)
                # cv2.imshow('BrightnessImage', dark_blur)
                # cv2.waitKey(0)
                nMask = copy.deepcopy(tempGT)
                bright_blur_mask = copy.deepcopy(tempsaliencyMask)
                bright_blur_mask[bright_blur_mask > 0] = 255
                alpha_mask = (copy.deepcopy(tempsaliencyMask) * 255.0) / 255.
                final_masked_blurred_image, y1o, y2o, x1o, x2o = overlay_image_alpha(copy.deepcopy(tempImage),
                                                                                     bright_blurred_overlay_img,
                ...
                nMask[np.argwhere(bright_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 0],
                      np.argwhere(bright_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 1]] = 4
                # save final image
                saveName = args.output_data_dir + "/images/"+size_name+"brightness_" + baseImageName+'_' + str(count)+ \
                           args.data_extension
                final_masked_blurred_image = np.round(np.power((np.array(final_masked_blurred_image)[:, :, 0:3] * 1.)
                                                               / 255.0, (1.0 / gamma)) * 255.0).astype(np.uint8)
                # https: // note.nkmk.me / en / python - opencv - bgr - rgb - cvtcolor /
                imageio.imwrite(saveName, final_masked_blurred_image)
                # create and save ground truth mask
                saveNameGt = args.output_data_dir + "/gt/"+size_name+"brightness_" + baseImageName + '_' + str(count) +\
                             args.data_extension
                nMask[nMask == 1] = 64
                nMask[nMask == 2] = 128
                nMask[nMask == 3] = 192
    ...
    #     baseImageName = images_list[i]
    #     # motion
    #     for kernal in motion_kernal_ranges:
    #         for angle in motion_degree_ranges:
    #             nMask = copy.deepcopy(gtOrigonal[i])
    #             # motion blur
    #             motion_blurred_overlay_img = apply_motion_blur(copy.deepcopy(saliencyImages[i]), kernal, angle)
    #             blur_mask = apply_motion_blur(copy.deepcopy(saliencyMask[i]) * 255, kernal, angle)
    ...
    #             nMask[np.argwhere(motion_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 0],
    #               np.argwhere(motion_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 1]] = 1
    #             saveName = args.output_data_dir + "/images/motion_" + baseImageName + '_angle_' + str(
    #                 angle) +"_kernal_"+ str(kernal)+ args.data_extension
    #             final_masked_blurred_image = np.round(np.power((np.array(final_masked_blurred_image)[:, :, 0:3] * 1.)
    #                                                        / 255.0, (1.0 / gamma)) * 255.0).astype(np.uint8)
    #             # https: // note.nkmk.me / en / python - opencv - bgr - rgb - cvtcolor /
    #             imageio.imwrite(saveName, final_masked_blurred_image)
    #             # create and save ground truth mask
    #             saveNameGt = args.output_data_dir + "/gt/motion_" + baseImageName + '_angle_' + str(angle) +"_kernal_"+ \
    #                      str(kernal)+ args.data_extension
    #             nMask[nMask == 1] = 64
    #             nMask[nMask == 2] = 128
    #             nMask[nMask == 3] = 192
    #             nMask[nMask == 4] = 255
    ...
    #         nMask[np.argwhere(focus_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 0],
    #               np.argwhere(focus_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 1]] = 2
    #         saveName = args.output_data_dir + "/images/focus_" + baseImageName + '_kernal_' + str(kernal) + \
    #                    args.data_extension
    #         final_masked_blurred_image = np.round(np.power((np.array(final_masked_blurred_image)[:, :, 0:3] * 1.)
    #                                                        / 255.0, (1.0 / gamma)) * 255.0).astype(np.uint8)
    #         # https: // note.nkmk.me / en / python - opencv - bgr - rgb - cvtcolor /
    #         imageio.imwrite(saveName, final_masked_blurred_image)
    #         # create and save ground truth mask
    #         saveNameGt = args.output_data_dir + "/gt/focus_" + baseImageName + '_kernal_' + str(kernal) + \
    #                    args.data_extension
    #         nMask[nMask == 1] = 64
    #         nMask[nMask == 2] = 128
    #         nMask[nMask == 3] = 192
    ...
    #             # create darkness blur
    #             nMask = copy.deepcopy(gtOrigonal[i])
    #             dark_blurred_overlay_img = create_brightness_and_darkness_blur(copy.deepcopy(saliencyImages[i]), alpha,
    #                                                                        beta)
    #             # cv2.imshow('BrightnessImage', dark_blur)
    #             # cv2.waitKey(0)
    #             dark_blur_mask = copy.deepcopy(saliencyMask[i])
    ...
    #             nMask[np.argwhere(dark_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 0],
    #               np.argwhere(dark_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 1]] = 3
    #             saveName = args.output_data_dir + "/images/darkness_" + baseImageName + '_alpha_' + str(alpha) + \
    ...
    #             imageio.imwrite(saveName, final_masked_blurred_image)
    #             # create and save ground truth mask
    #             saveNameGt = args.output_data_dir + "/gt/darkness_" + baseImageName + '_alpha_' + str(alpha) + \
    #                 "_beta_"+ str(beta) + args.data_extension
    #             nMask[nMask == 1] = 64
    #             nMask[nMask == 2] = 128
    #             nMask[nMask == 3] = 192
    ...
    #               np.argwhere(bright_blur_mask[y1o:y2o, x1o:x2o] == 255)[:, 1]] = 4
    #             # save final image
    #             saveName = args.output_data_dir + "/images/brightness_" + baseImageName + '_alpha_' + str(alpha) + \
    #                 "_beta_"+ str(beta) + args.data_extension
    #             final_masked_blurred_image = np.round(np.power((np.array(final_masked_blurred_image)[:, :, 0:3] * 1.)
    #                                                        / 255.0, (1.0 / gamma)) * 255.0).astype(np.uint8)
    #             # https: // note.nkmk.me / en / python - opencv - bgr - rgb - cvtcolor /
    #             imageio.imwrite(saveName, final_masked_blurred_image)
    #             # create and save ground truth mask
    #             saveNameGt = args.output_data_dir + "/gt/brightness_" + baseImageName + '_alpha_' + str(alpha) + \
    #                 "_beta_"+ str(beta) + args.data_extension
    #             nMask[nMask == 1] = 64
    #             nMask[nMask == 2] = 128
    #             nMask[nMask == 3] = 192
