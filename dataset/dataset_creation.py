...
# create training dataset from chuk where brightness and darkness are added
def create_dataset_for_training_synthetic_and_real(args):
    final_shape = (480, 640)
    tl.files.exists_or_mkdir(args.output_data_dir + "/images/")
    tl.files.exists_or_mkdir(args.output_data_dir + "/gt/")
    # list of all original Images
    ...
    images_darkness_list = sorted(tl.files.load_file_list(path=args.data_d_dir, regx='/*.(png|PNG)',printable=False))
    images_brightness_list = sorted(tl.files.load_file_list(path=args.data_b_dir, regx='/*.(png|PNG)', printable=False))
    images_darkness_real_list = sorted(tl.files.load_file_list(path=args.data_d_real_dir, regx='/*.(jpg|JPG)',
                                                               printable=False))
    images_brightness_real_list = sorted(tl.files.load_file_list(path=args.data_b_real_dir, regx='/*.(jpg|JPG)',
                                                                 printable=False))
    # SALIENCY
    saliency_images_list = sorted(tl.files.load_file_list(path=args.salinecy_data_dir + '/images', regx='/*.(jpg|JPG)',
                                                          printable=False))
    ...
    saliency_mask_d_list = sorted(tl.files.load_file_list(path=args.salinecy_data_d_dir, regx='/*.(png|PNG)',
                                                          printable=False))
    saliency_mask_b_real_list = sorted(tl.files.load_file_list(path=args.salinecy_data_b_real_dir, regx='/*.(png|PNG)',
                                                          printable=False))
    saliency_mask_d_real_list = sorted(tl.files.load_file_list(path=args.salinecy_data_d_real_dir, regx='/*.(png|PNG)',
                                                          printable=False))
    # these images have out of focus blur already we need to create motion blur, and the over and under exposure images
    imagesOrigonal = read_all_imgs(images_list, path=args.data_dir+'/images/', n_threads=100, mode='RGB')
    imagesDOrigonal = read_all_imgs(images_darkness_list, path=args.data_d_dir + '/',n_threads=100,mode='RGB')
    imagesBOrigonal = read_all_imgs(images_brightness_list, path=args.data_b_dir + '/',n_threads=100,mode='RGB')
    imagesDRealOrigonal = read_all_imgs(images_darkness_real_list, path=args.data_d_real_dir + '/', n_threads=100,
                                        mode='RGB')
    imagesBRealOrigonal = read_all_imgs(images_brightness_real_list, path=args.data_b_real_dir + '/', n_threads=100,
                                        mode='RGB')
    gtOrigonal = read_all_imgs(images_gt_list, path=args.data_dir+'/gt/', n_threads=100, mode='RGB2GRAY2')
    saliencyImages = read_all_imgs(saliency_images_list,path=args.salinecy_data_dir+'/images/',n_threads=100,mode='RGB')
    saliencyMask = read_all_imgs(saliency_mask_list, path=args.salinecy_data_dir+'/gt/',n_threads=100,mode='RGB2GRAY2')
    ...
    saliencyBMask = read_all_imgs(saliency_mask_b_list, path=args.salinecy_data_b_dir + '/', n_threads=100,
                                  mode='RGB2GRAY2')
    saliencyDRealMask = read_all_imgs(saliency_mask_d_real_list, path=args.salinecy_data_d_real_dir + '/',n_threads=100,
                                  mode='RGB2GRAY2')
    saliencyBRealMask = read_all_imgs(saliency_mask_b_real_list, path=args.salinecy_data_b_real_dir + '/',n_threads=100,
                                  mode='RGB2GRAY2')
    # original images
    for i in range(len(imagesOrigonal)):
        if len(gtOrigonal[i].shape) > 3:
    ...
    for i in range(len(imagesDOrigonal)):
        ...
        imagesDOrigonal[i] = 255.0 * np.power((new_image * 1.) / 255.0, gamma)
        #images_list[i] = images_list[i].split('.')[0]
        (T, saliency) = cv2.threshold(gtMask, (255/4)*3, 1, cv2.THRESH_BINARY)
        saliencyDMask[i] = saliency

    # darkness images
    for i in range(len(imagesDRealOrigonal)):
        if len(saliencyDRealMask[i].shape) > 3:
            saliencyDRealMask[i] = np.squeeze(saliencyDRealMask[i])[:, :, 0][:, :, np.newaxis]
        if imagesDRealOrigonal[i].shape[0] > imagesDRealOrigonal[i].shape[1]:
            imagesDRealOrigonal[i] = cv2.rotate(imagesDRealOrigonal[i],0)
            saliencyDRealMask[i] = cv2.rotate(saliencyDRealMask[i], 0)[:,:,np.newaxis]
        y1, y2 = max(0, int((final_shape[0] + 1 - imagesDRealOrigonal[i].shape[0]) / 2)), \
                 min(imagesDRealOrigonal[i].shape[0]+int((final_shape[0] + 1 - imagesDRealOrigonal[i].shape[0]) / 2),
                     final_shape[0])
        x1, x2 = max(0, int((final_shape[1] + 1 - imagesDRealOrigonal[i].shape[1]) / 2)), \
                 min(imagesDRealOrigonal[i].shape[1]+int((final_shape[1] + 1 - imagesDRealOrigonal[i].shape[1]) / 2),
                     final_shape[1])
        y1o, y2o = 0, min(imagesDRealOrigonal[i].shape[0], final_shape[0])
        x1o, x2o = 0, min(imagesDRealOrigonal[i].shape[1], final_shape[1])

        new_image = np.zeros((final_shape[0], final_shape[1], 3))
        new_image[y1:y2, x1:x2] = imagesDRealOrigonal[i][y1o:y2o,x1o:x2o]
        # darkness blur deafult for 0 pixels
        gtMask =  np.ones((final_shape[0], final_shape[1], 1))
        gtMask[y1:y2, x1:x2] = saliencyDRealMask[i][y1o:y2o,x1o:x2o]
        # gamma correction to have linear images
        imagesDRealOrigonal[i] = 255.0 * np.power((new_image * 1.) / 255.0, gamma)
        #images_list[i] = images_list[i].split('.')[0]
        (T, saliency) = cv2.threshold(gtMask, (255/4)*3, 1, cv2.THRESH_BINARY)
        saliencyDRealMask[i] = saliency
    ...
    # BRIGHTNESS images
    for i in range(len(imagesBRealOrigonal)):
        if len(saliencyBRealMask[i].shape) > 3:
            saliencyBRealMask[i] = np.squeeze(saliencyBRealMask[i])[:, :, 0][:, :, np.newaxis]
        if imagesBRealOrigonal[i].shape[0] > imagesBRealOrigonal[i].shape[1]:
            imagesBRealOrigonal[i] = cv2.rotate(imagesBRealOrigonal[i],0)
            saliencyBRealMask[i] = cv2.rotate(saliencyBRealMask[i], 0)#[:,:,np.newaxis]
        y1, y2 = max(0, int((final_shape[0] + 1 - imagesBRealOrigonal[i].shape[0]) / 2)), \
                 min(imagesBRealOrigonal[i].shape[0]+int((final_shape[0] + 1 - imagesBRealOrigonal[i].shape[0]) / 2),
                     final_shape[0])
        x1, x2 = max(0, int((final_shape[1] + 1 - imagesBRealOrigonal[i].shape[1]) / 2)), \
                 min(imagesBRealOrigonal[i].shape[1]+int((final_shape[1] + 1 - imagesBRealOrigonal[i].shape[1]) / 2),
                     final_shape[1])
        y1o, y2o = 0, min(imagesBRealOrigonal[i].shape[0], final_shape[0])
        x1o, x2o = 0, min(imagesBRealOrigonal[i].shape[1], final_shape[1])

        new_image = np.zeros((final_shape[0], final_shape[1], 3))
        new_image[y1:y2, x1:x2] = imagesBRealOrigonal[i][y1o:y2o,x1o:x2o]
        # darkness blur deafult for 0 pixels
        gtMask =  np.ones((final_shape[0], final_shape[1], 1))
        gtMask[y1:y2, x1:x2] = saliencyBRealMask[i][y1o:y2o,x1o:x2o]
        # gamma correction to have linear images
        imagesBRealOrigonal[i] = 255.0 * np.power((new_image * 1.) / 255.0, gamma)
        #images_list[i] = images_list[i].split('.')[0]
        (T, saliency) = cv2.threshold(gtMask, (255/4)*3, 1, cv2.THRESH_BINARY)
        saliencyBRealMask[i] = saliency
    ...
    saliencyDarkIdx = np.arange(start=0, stop=len(saliencyDMask), step=1)
    saliencyBrightIdx = np.arange(start=0, stop=len(saliencyBMask), step=1)
    saliencyDarkRealIdx = np.arange(start=0, stop=len(saliencyDRealMask), step=1)
    saliencyBrightRealIdx = np.arange(start=0, stop=len(saliencyBRealMask), step=1)
    list_choice = [0,1]
    # this already makes a huge dataset
    for i in range(len(imagesOrigonal)):
        baseImageName = images_list[i]
        ...
        for count in range(5):
            ...
            for j in idxs:
                ...
                # darkness blur
                    ...
                    # decide if randomly if we want to use real image or create the blur
                    # https://pynative.com/python-weighted-random-choices-with-probability/
                    x = random.choice(list_choice)
                    if x == 0:
                        # create darkness blur
                        overlayImageIdx = np.random.choice(saliencyDarkIdx, 1, replace=False)[0]
                        alpha,beta = random_darkness_value()
                        ...
                        # create darkness blur
                        overlayImageIdx = np.random.choice(saliencyDarkRealIdx, 1, replace=False)[0]
                        # alpha,beta = random_darkness_value()
                        # dark_blurred_overlay_img = create_brightness_and_darkness_blur(copy.deepcopy(saliencyImages[overlayImageIdx]),
                        #                                                            alpha, beta)
                        # cv2.imshow('BrightnessImage', dark_blur)
                        # cv2.waitKey(0)
                        dark_blur_mask = copy.deepcopy(saliencyDRealMask[overlayImageIdx])
                        dark_blur_mask[dark_blur_mask > 0] = 255
                        alpha_mask = (copy.deepcopy(saliencyDRealMask[overlayImageIdx]) * 255.0) / 255.
                        # cv2.imshow('BrightnessImage_mask', dark_blur_masked)
                        # cv2.waitKey(0)
                        placement = (np.random.randint(-final_shape[0] * .75, final_shape[0] * .75, 1)[0],
                                     np.random.randint(-final_shape[1] * .75, final_shape[1] * .75, 1)[0])
                ...
                # brightness blur
                    ...
                    if x == 0:
                        ...
                        # create brightness blur
                        overlayImageIdx = np.random.choice(saliencyBrightRealIdx, 1, replace=False)[0]
                        # alpha, beta = random_brightness_value()
                        # bright_blurred_overlay_img = create_brightness_and_darkness_blur(
                        #     copy.deepcopy(saliencyImages[overlayImageIdx]), alpha, beta)
                        # cv2.imshow('BrightnessImage', dark_blur)
                        # cv2.waitKey(0)
                        bright_blur_mask = copy.deepcopy(saliencyBRealMask[overlayImageIdx])
                        bright_blur_mask[bright_blur_mask > 0] = 255
                        alpha_mask = (copy.deepcopy(saliencyBRealMask[overlayImageIdx]) * 255.0) / 255.
                        placement = (np.random.randint(-final_shape[0] * .75, final_shape[0] * .75, 1)[0],
                                     np.random.randint(-final_shape[1] * .75, final_shape[1] * .75, 1)[0])
                        final_masked_blurred_image, y1o, y2o, x1o, x2o = overlay_image_alpha(final_masked_blurred_image,
                                                                                             copy.deepcopy(
            ...
            if np.isnan(final_masked_blurred_image).any():
                print('i went here')
                final_masked_blurred_image[final_masked_blurred_image == np.nan] = 0
            # https: // note.nkmk.me / en / python - opencv - bgr - rgb - cvtcolor /
            imageio.imwrite(saveName, final_masked_blurred_image)
            # create and save ground truth mask
            saveNameGt = args.output_data_dir + "/gt/" + baseImageName + '_' + str(count) + args.data_extension
...
# create testing dataset for chuk with brightness and darkness images
def create_dataset_for_testing_2(args):
    final_shape = (480, 640)
    tl.files.exists_or_mkdir(args.output_data_dir + "/images/")
    tl.files.exists_or_mkdir(args.output_data_dir + "/gt/")
    # list of all original Images
    ...
    for i in range(len(imagesBOrigonal)):
        ...
        gt[gt > 0] = 4
        gt[gt == 0] = 2
        gtMask[y1:y2, x1:x2] = gt#saliencyBMask[i][y1o:y2o, x1o:x2o]
        # gamma correction to have linear images
        imagesBOrigonal[i] = 255.0 * np.power((new_image * 1.) / 255.0, gamma)
        # images_list[i] = images_list[i].split('.')[0]
        #(T, saliency) = cv2.threshold(gtMask, 1, 1, cv2.THRESH_BINARY)
...
# create testing dataset for chuk with brightness and darkness images
def create_dataset_for_testing_my_real_bd_images_test3(args):
    final_shape = (480, 640)
    tl.files.exists_or_mkdir(args.output_data_dir + "/images/")
    tl.files.exists_or_mkdir(args.output_data_dir + "/gt/")
    # list of all original Images
    images_darkness_list = sorted(tl.files.load_file_list(path=args.data_d_dir, regx='/*.(jpg|JPG)', printable=False))
    images_brightness_list = sorted(tl.files.load_file_list(path=args.data_b_dir, regx='/*.(jpg|JPG)', printable=False))
    saliency_mask_b_list = sorted(tl.files.load_file_list(path=args.salinecy_data_b_dir, regx='/*.(png|PNG)',
                                                          printable=False))
    saliency_mask_d_list = sorted(tl.files.load_file_list(path=args.salinecy_data_d_dir, regx='/*.(png|PNG)',
                                                          printable=False))
    ...
    for i in range(len(imagesDOrigonal)):
        # crop the images
        # if '2object' in images_darkness_list[i]:
        #     imagesDOrigonal[i] = imagesDOrigonal[i][255:,200:500]
        #     saliencyDMask[i] = saliencyDMask[i][255:,200:500]
        # elif '5object' in images_darkness_list[i]:
        #     imagesDOrigonal[i] = imagesDOrigonal[i][150:, 210:]
        #     saliencyDMask[i] = saliencyDMask[i][150:, 210:]
        # else:
        #     imagesDOrigonal[i] = imagesDOrigonal[i][:,140:500]
        #     saliencyDMask[i] = saliencyDMask[i][:,140:500]
        if len(saliencyDMask[i].shape) > 3:
            saliencyDMask[i] = np.squeeze(saliencyDMask[i])[:,:,0][:,:,np.newaxis]
        elif len(saliencyDMask[i].shape) < 3:
            saliencyDMask[i] = saliencyDMask[i][:, :, np.newaxis]
        if imagesDOrigonal[i].shape[0] > imagesDOrigonal[i].shape[1]:
            imagesDOrigonal[i] = cv2.rotate(imagesDOrigonal[i], 0)
            saliencyDMask[i] = cv2.rotate(saliencyDMask[i], 0)[:, :, np.newaxis]
        y1, y2 = max(0, int((final_shape[0] + 1 - imagesDOrigonal[i].shape[0]) / 2)), \
        ...
        imagesDOrigonal[i] = 255.0 * np.power((new_image * 1.) / 255.0, gamma)
        # images_list[i] = images_list[i].split('.')[0]
        (T, saliency) = cv2.threshold(saliencyDMask[i], 255/2, 3, cv2.THRESH_BINARY)
        saliency[y1o:y2o, x1o:x2o][saliency[y1o:y2o, x1o:x2o] == 0] = 2 # focus
        gtMask[y1:y2, x1:x2] = saliency[y1o:y2o, x1o:x2o,np.newaxis]
        saliencyDMask[i] = gtMask

    # BRIGHTNESS images
    for i in range(len(imagesBOrigonal)):
        # if '2object' in images_brightness_list[i]:
        #     imagesBOrigonal[i] = imagesBOrigonal[i][255:,200:500]
        #     saliencyBMask[i] = saliencyBMask[i][255:,200:500]
        # elif '5object' in images_brightness_list[i]:
        #     imagesBOrigonal[i] = imagesBOrigonal[i][150:, 210:]
        #     saliencyBMask[i] = saliencyBMask[i][150:, 210:]
        # else:
        #     imagesBOrigonal[i] = imagesBOrigonal[i][:,140:500]
        #     saliencyBMask[i] = saliencyBMask[i][:,140:500]
        if len(saliencyBMask[i].shape) > 3:
            saliencyBMask[i] = np.squeeze(saliencyBMask[i])[:,:,0][:,:,np.newaxis]
        if imagesBOrigonal[i].shape[0] > imagesBOrigonal[i].shape[1]:
            imagesBOrigonal[i] = cv2.rotate(imagesBOrigonal[i], 0)
        ...
        # darkness blur deafult for 0 pixels
        gtMask = np.ones((final_shape[0], final_shape[1], 1))*3
        if len(saliencyBMask[i].shape) == 2:
            saliencyBMask[i] = saliencyBMask[i][:, :, np.newaxis]
        #gtMask[y1:y2, x1:x2] = saliencyBMask[i][y1o:y2o, x1o:x2o]
        # gamma correction to have linear images
        imagesBOrigonal[i] = 255.0 * np.power((new_image * 1.) / 255.0, gamma)
        # images_list[i] = images_list[i].split('.')[0]
        (T, saliency) = cv2.threshold(saliencyBMask[i], 255/2, 4, cv2.THRESH_BINARY)
        saliency[y1o:y2o, x1o:x2o][saliency[y1o:y2o, x1o:x2o] == 0] = 2  # focus
        gtMask[y1:y2, x1:x2] = saliency[y1o:y2o, x1o:x2o,np.newaxis]
        saliencyBMask[i] = gtMask
    ...
    for i in range(len(imagesDOrigonal)):
        # always remember gamma correction
        baseImage = np.round(np.power((imagesDOrigonal[i] * 1.) / 255, (1 / gamma)) * 255.0).astype(np.uint8)
        baseImageName = images_darkness_list[i]
        saveName = args.output_data_dir + "/images/" + baseImageName + '_' + str(count) + args.data_extension
        imageio.imsave(saveName, baseImage)
        # get ground truth mask
    ...
    for i in range(len(imagesBOrigonal)):
        # always remember gamma correction
        baseImage = np.round(np.power((imagesBOrigonal[i] * 1.) / 255, (1 / gamma)) * 255.0).astype(np.uint8)
        baseImageName = images_brightness_list[i]
        saveName = args.output_data_dir + "/images/" + baseImageName + '_' + str(count) + args.data_extension
        imageio.imsave(saveName, baseImage)
        # get ground truth mask
...
# create testing dataset for chuk with brightness and darkness images
def create_dataset_for_testing_my_real_bd_images_test4(args):
    final_shape = (480, 640)
    tl.files.exists_or_mkdir(args.output_data_dir + "/images/")
    tl.files.exists_or_mkdir(args.output_data_dir + "/gt/")
    # list of all original Images
...
# create testing dataset for chuk with brightness and darkness images
def create_dataset_for_testing_my_real_bd_images_test2(args):
    final_shape = (480, 640)
    tl.files.exists_or_mkdir(args.output_data_dir + "/images/")
    tl.files.exists_or_mkdir(args.output_data_dir + "/gt/")
    # list of all original Images
...
# create testing dataset for chuk with brightness and darkness images
def create_dataset_for_testing_my_real_bd_images_test1(args):
    final_shape = (480, 640)
    tl.files.exists_or_mkdir(args.output_data_dir + "/images/")
    tl.files.exists_or_mkdir(args.output_data_dir + "/gt/")
    # list of all original Images
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_d_dir', type=str, default=None)
    parser.add_argument('--data_b_dir', type=str, default=None)
    parser.add_argument('--data_d_real_dir', type=str, default=None)
    parser.add_argument('--data_b_real_dir', type=str, default=None)
    ...
    parser.add_argument('--salinecy_data_d_real_dir', type=str, default=None)
    parser.add_argument('--salinecy_data_b_real_dir', type=str, default=None)
    ...
    if not args.is_testing:
        create_dataset_for_training_synthetic_and_real(args)
        #create_dataset_for_training(args)
    else:
        create_dataset_for_testing_my_real_bd_images_test4(args)
    # if args.is_testing:
