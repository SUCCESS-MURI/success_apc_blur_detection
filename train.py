...
def train_with_chuk_updated_dataset():
    ...
    # else:
    #     numValid = 290#int(len(train_blur_imgs) * .1)
...
# my new code for training the data using this version.
def train_with_ips_dataset():
    checkpoint_dir = "model/{}".format(tl.global_flag['mode'])  # checkpoint_resize_conv
    tl.files.exists_or_mkdir(checkpoint_dir)
    log_config(checkpoint_dir + '/config', config)
    ...
    train_blur_img_list = sorted(tl.files.load_file_list(path=input_path, regx='/*.(png|PNG)', printable=False))
    train_mask_img_list = sorted(tl.files.load_file_list(path=gt_path, regx='/*.(png|PNG)', printable=False))
    input_path_val = config.VALIDATION.blur_path
    gt_path_val = config.VALIDATION.gt_path
    val_blur_img_list = sorted(tl.files.load_file_list(path=input_path_val, regx='/*.(png|PNG)', printable=False))
    val_mask_img_list = sorted(tl.files.load_file_list(path=gt_path_val, regx='/*.(png|PNG)', printable=False))

    ###Load Training Data ####
    train_blur_imgs = read_all_imgs(train_blur_img_list, path=input_path, n_threads=100, mode='RGB')
    train_mask_imgs = read_all_imgs(train_mask_img_list, path=gt_path, n_threads=100, mode='RGB2GRAY')
    val_blur_imgs = read_all_imgs(val_blur_img_list, path=input_path, n_threads=100, mode='RGB')
    val_mask_imgs = read_all_imgs(val_mask_img_list, path=gt_path, n_threads=100, mode='RGB2GRAY')
    list_names_idx = np.arange(0, len(train_blur_imgs), 1)
    np.random.shuffle(list_names_idx)
    # need to know which images were in the valid dataset
    train_classification_mask = []
    ...
    train_blur_imgs = np.array(train_blur_imgs)
    train_classification_mask = np.array(train_classification_mask)
    valid_blur_imgs = np.array(val_blur_imgs)
    ...
    mean_accuracy_per_class = np.mean(np.array(accuracy_list), axis=0)
    ## save model
    if np.mean(mean_accuracy_per_class) > best_accuracy:
        tl.files.save_ckpt(sess=sess, mode_name='SA_net_{}_best_model.ckpt'.format(tl.global_flag['mode']),
                           save_dir=checkpoint_dir, var_list=a_vars, printable=False)
        best_accuracy = np.mean(mean_accuracy_per_class)
    log = "[*] Validation Results: time: %4.4fs, total_err: %.8f Mean Accuracy: %.8f Accuracy per class: 0: %.8f, 1: %.8f, 2: %.8f, 3: %.8f, 4: %.8f\n" % (
        time.time() - step_time,
        total_loss / n_iter, np.mean(mean_accuracy_per_class), mean_accuracy_per_class[0], mean_accuracy_per_class[1],
        mean_accuracy_per_class[2],
        mean_accuracy_per_class[3], mean_accuracy_per_class[4])
    # only way to write to log file while running
    with open(checkpoint_dir + "/training_ssc_metrics.log", "a") as f:
        # perform file operations
    ...
    with open(checkpoint_dir + "/validation_metrics.csv", "a") as f:
        writer = csv.writer(f)
        writer.writerow(
            [str(0), str(np.round(total_loss / n_iter, 8)), str(np.round(mean_accuracy_per_class[0], 8)),
             str(np.round(mean_accuracy_per_class[1], 8)), str(np.round(mean_accuracy_per_class[2], 8)),
             str(np.round(mean_accuracy_per_class[3], 8)), str(np.round(mean_accuracy_per_class[4], 8))])
    ...
    for epoch in range(tl.global_flag['start_from'], n_epoch + 1):
        ...
        for idx in range(0, len(train_blur_imgs), batch_size):
            ...
            err, _, _, _, _, _, outmap = sess.run([loss, loss1, loss2, loss3, loss4, train_op, out],
                                                  {net_regression.inputs: imlist,
                                                   classification_map: clist})
            total_loss += err
            n_iter += 1
        ...
        # check and do validation if necessary
        if epoch % do_validation_every == 0:
            total_loss, n_iter = 0, 0
            new_batch_size = batch_size
            step_time = time.time()
            accuracy_list = []
            ...
            log = "[*] Validation Results: time: %4.4fs, total_err: %.8f Mean Accuracy: %.8f Accuracy per class: 0: %.8f, 1: %.8f, 2: %.8f, 3: %.8f, 4: %.8f\n" % (
                time.time() - step_time,
                total_loss / n_iter, np.mean(mean_accuracy_per_class), mean_accuracy_per_class[0],
                mean_accuracy_per_class[1], mean_accuracy_per_class[2],
                mean_accuracy_per_class[3], mean_accuracy_per_class[4])
            # only way to write to log file while running
            with open(checkpoint_dir + "/training_ssc_metrics.log", "a") as f:
                # perform file operations
