...
def train_with_CUHK():
    ...
    imagesmaskZipped = tl.prepro.threading_data([_ for _ in zip(train_blur_imgs,train_classification_mask)],
                                                fn=crop_sub_img_and_classification_fn)
    # print images_and_score.shape
    train_blur_imgs, train_classification_mask = imagesmaskZipped.transpose((1, 0, 2, 3, 4))
    # print clist.shape
    train_classification_mask = train_classification_mask[:, :, :, 0]
    # print clist.shape
    train_classification_mask = np.expand_dims(train_classification_mask, axis=3)
    for epoch in range(tl.global_flag['start_from'], n_epoch + 1):
        ...
        for idx in range(0, len(train_blur_imgs), new_batch_size):
            ...
            if augmentation == 0:
                ...
                clist = train_classification_mask[idx: idx + new_batch_size]
            elif augmentation == 1:
